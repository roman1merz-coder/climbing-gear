...
def extract_products_from_html(html):
    ...
    for box in boxes:
        ...
        # Current layout: <a href="URL" title="NAME" class="product-name">
        # (href and title come BEFORE class="product-name")
        name_match = re.search(
            r'<a\s+href="([^"]+)"\s*(?:\n\s*)title="([^"]+)"\s*(?:\n\s*)class="product-name"',
            box
        )
        if name_match:
            ...
            # Fallback: any <a> with class="product-name" regardless of attribute order
            name_match = re.search(
                r'<a\s+[^>]*href="([^"]+)"[^>]*class="product-name"[^>]*title="([^"]+)"',
                box
            )
            if not name_match:
                name_match = re.search(
                    r'<a\s+[^>]*class="product-name"[^>]*href="([^"]+)"[^>]*title="([^"]+)"',
                    box
                )
            if name_match:
                url = name_match.group(1)
        ...
        # Brand — may be plain text in manufacturer-col div or wrapped in <a>
        brand_match = re.search(
            r'product-box-manufacturer-col[^>]*>\s*(?:<a[^>]*>)?\s*([A-Za-z][^<\n]+)',
            box
        )
        if not brand_match:
...
def fetch_product_sizes(url):
    """Fetch product detail page and extract available EU sizes from <label> elements.
    
    Primary strategy: Extract sizes from <label> elements with pattern:
    <label[^>]*>\s*(\d{2}[.,]?\d?)\s*</label>
    
    Fallback strategy: Extract from <option> elements in select dropdowns
    
    Normalizes comma decimals to dots: "35,5" → "35.5"
    Returns sorted list of EU size strings.
    """
    try:
        html = fetch_html(url)
        sizes = set()

        # Strategy 1: Extract from <label> elements (German shops)
        label_matches = re.findall(r'<label[^>]*>\s*(\d{2}[.,]?\d?)\s*</label>', html)
        for match in label_matches:
            # Normalize comma to dot
            normalized = match.replace(',', '.')
            sizes.add(normalized)

        # Strategy 2: Fallback to <option> elements in select dropdowns
        if not sizes:
            select_match = re.search(r'<select[^>]*id="[^"]*size[^"]*"[^>]*>(.*?)</select>', html, re.DOTALL | re.I)
            if not select_match:
                select_match = re.search(r'<select[^>]*name="[^"]*size[^"]*"[^>]*>(.*?)</select>', html, re.DOTALL | re.I)
            if select_match:
                opts = re.findall(r'<option[^>]*value="[^"]*"[^>]*>\s*([^<]+?)\s*</option>', select_match.group(1))
                for o in opts:
                    clean = o.split(' - ')[0].strip()
                    if re.match(r'^\d{2}([.,]\d{1,2})?$', clean):
                        if 'nicht' not in o.lower() and 'unavail' not in o.lower() and 'sold' not in o.lower():
                            sizes.add(clean.replace(',', '.'))
