...
- Heel view: heel_medial and heel_lateral should be at roughly the same y-coordinate.
...
export default async function handler(req, res) {
  ...
  try {
        ...
        messages: [
          {
            ...
            content: [
              ...
              { type: "text", text: "IMAGE 1 OF 3: TOP VIEW (phone above foot, looking straight down)" },
              { type: "image", source: { type: "base64", media_type: "image/jpeg", data: images.side } },
              { type: "text", text: "IMAGE 2 OF 3: SIDE VIEW (phone horizontal on floor beside foot, medial profile)" },
    ...
    const validationErrors = validateLandmarks(top, side, heel);
    ...
    // HEEL VIEW: heel width ratio
    // Compute from heel view using aspect ratio approach, then map to forefoot-relative scale
...
function validateLandmarks(top, side, heel) {
  const errors = [];
  ...
  const topKeys = ['toe_1','toe_2','toe_3','toe_4','toe_5','met_tibiale','met_fibulare','pternion'];
  ...
  // Validate all heel view landmarks exist
  const heelKeys = ['heel_medial','heel_lateral','achilles_mid'];
  for (const k of heelKeys) {
    if (!heel?.[k]) { errors.push(`heel.${k}: missing`); }
    else { isValid(heel[k], `heel.${k}`); }
  }
  ...
  // Instep apex: must be above ground but not too high (ankle/leg confusion)
  ...
  // Heel posterior should be far from mtp_joint (it's at the back of the foot)
  ...
  // --- Heel view geometric checks ---
  const heelYDiff = Math.abs(heel.heel_medial[1] - heel.heel_lateral[1]);
  const heelWidth = Math.abs(heel.heel_medial[0] - heel.heel_lateral[0]);
  if (heelWidth > 0 && heelYDiff / heelWidth > 0.5) {
    errors.push(`heel: heel_medial and heel_lateral y-coordinates differ significantly — they should be at roughly the same height`);
  }
