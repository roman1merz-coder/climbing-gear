...
def crashpad_pros(p):
    ...
    portability = p.get("portability_score", 50)
    ...
    # Carry system
    if portability >= 70:
        pros.append(f"Excellent portability ({portability}/100) — comfortable on long approaches")
    elif portability >= 55:
        pros.append(f"Good portability ({portability}/100) — handles moderate approaches well")
    if waist and chest:
...
def crashpad_cons(p):
    ...
    # Poor carry
    if portability < 40:
        cons.append(f"Low portability ({portability}/100) — best for roadside boulders")
    elif portability < 50:
        cons.append(f"Below-average portability ({portability}/100) — manageable for short walks only")
...
def crashpad_voices(p):
    ...
    if portability >= 70:
        ...
        pool.append("This pad is meant for roadside bouldering. Don't expect to hike with it.")
